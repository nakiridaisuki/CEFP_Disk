...
@twofa_api.route('/api/auth/twofa/setup', methods=['GET'])
@jwt_required()
def twofa_setup():
  """
  Setup 2FA
...
@twofa_api.route('/api/auth/twofa/check', methods=['GET'])
@jwt_required()
def twofa_check():
  """
  Check 2FA
  ...
  if user.totp_secret is None:
    return standard_response(
      data={ 'hasTwoFactorAuth': False }
    )
  else:
    return standard_response(
      data={ 'hasTwoFactorAuth': True }
    )
  
@twofa_api.route('/api/auth/twofa/cancel', methods=['GET'])
@jwt_required()
def twofa_cancel():
  """
  Cancel 2FA
